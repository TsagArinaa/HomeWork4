...
public class HomeWork4 {
    ...
    public static List<String> correctedList(List<String> stringList) {
        List<String> stringArrayList_Corr = new ArrayList<>();
        for (String elem : stringList) {
            if(!elem.matches(".*[A-Z].*")){
                stringArrayList_Corr.add(elem);
            }
        }
        return stringArrayList_Corr;

    }
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
       HashMap<String, Integer> hashMapCheck = new HashMap<>();
        for (Map.Entry<String, Integer> entry :HASH_MAP.entrySet()) {
            if (entry.getValue() <= 300) hashMapCheck.put(entry.getKey(), entry.getValue());
        }
        return hashMapCheck;
    }
